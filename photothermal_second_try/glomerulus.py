...
# %% -----
with open(path + "Glomerulus_05-03-2024_4.npy", "rb") as f:
    data = np.load(f, allow_pickle=True)
pt = data["pt"][0].astype(np.int16)
mir = data["mir"][0].astype(np.int16)

# mir = (mir - 2047.5) / 2047.5
# pt = (pt - 2047.5) / 2047.5
...
# %% -----
xlim = (0.01103383519771072, 0.012643486602497402)
(idx_xlim,) = np.logical_and(xlim[0] < freq, freq < xlim[1]).nonzero()

for idx in tqdm(range(mir.shape[0])):
    # ----- calculate phase correction
    ft = rfft(ifftshift(mir[idx]))
    p = np.unwrap(np.angle(ft))
    polyfit = np.polyfit(freq[idx_xlim], p[idx_xlim], deg=2)
    poly1d = np.poly1d(polyfit[:])

    # ----- apply phase correction to channel 2
    ft *= np.exp(-1j * poly1d(freq))
    ...
    if idx == 2:
        fig, ax = plt.subplots(1, 1)
        (l1,) = ax.plot(mir[idx], animated=True)
        (l2,) = ax.plot(mir[idx], animated=True)
        bm = blit.BlitManager(fig.canvas, [l1, l2])
        bm.update()
    elif idx > 2:
        l2.set_ydata(mir[idx])
        bm.update()
